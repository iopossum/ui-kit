    ...
    if ((isComponent && !isIndex) || !isComponent) {
      config.push({
        ...
        output: [
          {
            file: isComponent
              ? `dist/${folder}/${files[i]}/index.js`
              : `dist/${folder}/${files[i].replace(/ts$/, 'js')}`,
            format: 'esm',
            sourcemap: false,
          },
        ],
      ...
      config.push({
        input: isComponent
          ? `dist/types/${folder}/${files[i]}/index.d.ts`
          : `dist/types/${folder}/${files[i].replace(/ts$/, 'd.ts')}`,
        output: [
          {
            file: isComponent
              ? `dist/${folder}/${files[i]}/index.d.ts`
              : `dist/${folder}/${files[i].replace(/ts$/, 'd.ts')}`,
            format: 'esm',
            sourcemap: false,
          },
        ],
...
const config = [
  {
    input,
    output: [{ file: `dist/${pkg.main}`, format: 'cjs', sourcemap: true, name: 'core' }],
    plugins: [
      externals(),
      nodeResolve(),
      commonJs(),
  ...
  {
    input,
    output: [{ file: `dist/${pkg.module}`, format: 'esm', sourcemap: false, assetFileNames: '[name][extname]' }],
    plugins: [
      alias(aliasProps),
      externals(),
      nodeResolve(),
  ...
  },
  {
    ...
    output: [{ file: 'dist/cjs/utils.js', format: 'cjs', sourcemap: false, name: 'utils' }],
    plugins: [externals(), nodeResolve(), commonJs(), cleanup(), typescript(typescriptProps), json(), terser()],
    external,
  },
  {
    input: './src/stores/index.ts',
    output: [{ file: 'dist/cjs/stores.js', format: 'cjs', sourcemap: false, name: 'stores' }],
    plugins: [
      alias(aliasProps),
      externals(),
      nodeResolve(),
  ...
  },
  {
    ...
    output: [{ file: 'dist/cjs/hooks.js', format: 'cjs', sourcemap: false, name: 'hooks' }],
    plugins: [
      alias(aliasProps),
      externals(),
      nodeResolve(),
